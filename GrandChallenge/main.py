...
from utilities.email01 import send_email
    ...
    try: 
        ...
        while CURRENT_STATE != END:
            ...
            if CURRENT_STATE == GET_NEXT_GOAL:
                ...
                for block in blocks:
                    if (block.color == target_color and not block.knocked_over):
                ...
                while not block_collected:
                    ...
                    if target_block.bounding_origin[1] >= BLOCK_IN_GRIP_RANGE: # if bounding box is low in frame
                        if target_block.angle_to_robo < 10:
                            ...
                            rotation_angle = rmc.rotate_by(goal_angle * 0.8)
        ...
        print("[MAIN] Interrupted! Releasing threads.")
        # State is now END
        rmc.stop_motion()
        odom.kill()
        imu.kill()
        world_map.kill()
        print("[Main] Program execution ended with interupation.")